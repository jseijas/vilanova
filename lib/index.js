...
class Vilanova {
  ...
  static addDefaultTypes() {
    ...
    Vilanova.addType('String', value => `#String:${value}`, value => value);
  }

  static getType(obj, key, value) {
    // eslint-disable-next-line
    if (typeof value === 'bigint') {
      return Vilanova.types.BigInt;
    }
    return undefined;
  }
  ...
  static replacer(key, value) {
    const type = Vilanova.getType(this, key, value);
    if (type) {
      return type.replacer(value);
    }
    const token = Vilanova.getToken(value);
    if (token.type && Vilanova.types[token.type]) {
      return `#String:${value}`;
    }
    return value;
  }
